...
helper.mix(AppRouter, utilities);
...
*/
document.registerElement("route-view", {
...
AppRouter.init = function() {
  ...
  if (router.hasAttribute("core-animated-pages")) {
    ...
    router.coreAnimatedPages.setAttribute(
      "transitions",
      router.getAttribute("transitions")
    );
    ...
    router.coreAnimatedPages.addEventListener(
      ...
      function() {
        ...
        if (
          ...
          !router.previousRoute.hasAttribute("active")
        ) {
...
function stateChange(router) {
  let url = utilities.parseUrl(
    window.location.href,
    router.getAttribute("mode")
  );
...
function activateRoute(router, route, url) {
  ...
  if (
    ...
    route.getAttribute("onUrlChange") === "noop"
  ) {
    return;
  }
  ...
  if (
    route === router.activeRoute &&
    route.getAttribute("onUrlChange") === "updateModel"
  ) {
  ...
  // inline template
    ...
    route.firstElementChild &&
    route.firstElementChild.tagName === "TEMPLATE"
  ) {
...
function activateImport(
  ...
  if (route === router.loadingRoute) {
    if (route.hasAttribute("template")) {
      // template
      let templateId = route.getAttribute("template");
      let template;
      if (templateId) {
...
function activateCustomElement(router, elementName, route, url, eventDetail) {
  let customElement = document.createElement(elementName);
  let model = createModel(router, route, url, eventDetail);

  AppRouter.currentRoute = model;
  // debug("model", model);

  activateElement(router, customElement, url, eventDetail);

  fire("routeChangeSuccess", model, router);
}
...
function createModel(router, route, url, eventDetail) {
  let model = utilities.routeArguments(
    route.getAttribute("path"),
    url.path,
    url.search,
    route.hasAttribute("regex"),
 ...
 * 注册路由组件
 */
window.document.registerElement("router-view", {
  prototype: AppRouter
});
